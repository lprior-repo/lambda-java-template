# Serverless Infrastructure - Java Lambda Template
...
module "health_lambda" {
  ...
  handler       = "com.example.health.HealthHandler"
  runtime       = "java21"
  architectures = ["arm64"]
  memory_size   = 256
  timeout       = 15
  ...
  local_existing_package = "../build/health-lambda.jar"
...
module "users_lambda" {
  ...
  handler       = "com.example.users.UsersHandler"
  runtime       = "java21"
  architectures = ["arm64"]
  memory_size   = 512
  timeout       = 30
  ...
  create_package         = false
  local_existing_package = "../build/users-lambda.jar"
...
module "posts_lambda" {
  ...
  handler       = "com.example.posts.PostsHandler"
  runtime       = "java21"
  architectures = ["arm64"]
  memory_size   = 512
  timeout       = 30
  ...
  create_package         = false
  local_existing_package = "../build/posts-lambda.jar"
...
module "event_processor_lambda" {
  ...
  handler       = "com.example.processor.EventProcessorHandler"
  runtime       = "java21"
  architectures = ["arm64"]
  memory_size   = 256
  timeout       = 30
  ...
  create_package         = false
  local_existing_package = "../build/event-processor-lambda.jar"
