...
COPY src/product-service/target/product-service.jar .
...
if [ -f "docker-output/product-service-native.zip" ]; then
    cp docker-output/product-service-native.zip src/product-service/target/
    echo "Successfully created Lambda-compatible native package!"
    echo "Package size: $(du -h src/product-service/target/product-service-native.zip | cut -f1)"
else
